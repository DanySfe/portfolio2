...
if (themeToggle) {
  ...
  /* --------- Canvas animated background: Connected Dots (Plexus) --------- */
  (function particleNetwork() {
    ...
    // Mouse interaction for connection
    ...
    function setupParticles() {
      ...
      const color = isLight ? '255, 204, 0' : '160, 77, 255'; // Theme color
    ...
    function draw() {
      ...
      const isLight = document.documentElement.classList.contains('light-mode');
      const baseColor = isLight ? '255, 204, 0' : '160, 77, 255';
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (mouse.x) {
          ...
          let dist = Math.sqrt(dx * dx + dy * dy);
          if (dist < 150) {
            ctx.strokeStyle = `rgba(${baseColor}, ${(1 - dist / 150) * 0.4})`;
            ctx.lineWidth = 0.8;
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
    ...
    document.addEventListener('themechange', setupParticles);
